...
PIPEWIRE_VERSION = 83ada1e086f8fa49bf9652df2edd9ccee5efa48b
...
define PIPEWIRE_INSTALL_ALSA_CONF
	$(INSTALL) -d $(TARGET_DIR)/etc/alsa/conf.d/
	ln -sf /usr/share/alsa/alsa.conf.d/50-pipewire.conf $(TARGET_DIR)/etc/alsa/conf.d/50-pipewire.conf
	$(INSTALL) -m 644 $(TARGET_DIR)/usr/share/alsa/alsa.conf.d/99-pipewire-default.conf $(TARGET_DIR)/etc/alsa/conf.d/99-pipewire-default.conf
endef
PIPEWIRE_POST_INSTALL_TARGET_HOOKS += PIPEWIRE_INSTALL_ALSA_CONF
else
...
define PIPEWIRE_INSTALL_INIT_SYSV
	mkdir -p $(TARGET_DIR)/etc/pipewire/pipewire.conf.d
	mkdir -p $(TARGET_DIR)/etc/profile.d/
	$(INSTALL) -D -m 755 package/pipewire/S45pipewire $(TARGET_DIR)/etc/init.d/S45pipewire
	$(INSTALL) -D -m 755 package/pipewire/pw.sh $(TARGET_DIR)/etc/profile.d/pw.sh
	$(INSTALL) -D -m 644 package/pipewire/$(BR2_PACKAGE_PIPEWIRE_DEFAULT_SINK_CONF) $(TARGET_DIR)/etc/pipewire/pipewire.conf.d/10-my-default-sink.conf
endef
