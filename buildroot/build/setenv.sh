...
BUILDROOT_EXTERNAL_DIR=$LOCAL_DIR/br_project
$LOCAL_DIR/br-aml/build/patch-open-buildroot.sh 
FILTER=$1
if [[ "$FILTER" =~ "_" ]]
then
    FILTER=""
    ...
    DEFAULT_TARGET=$(pushd $BUILDROOT_DIR/configs 2>&1 >> /dev/null; find -name '*_release_defconfig' | sed 's@./@@' | sed 's@_defconfig@@' | grep $DEFAULT_TARGET | sort | head -1 )
else
    DEFAULT_TARGET=""
    ALL_FLAG=$1
    FILTER=$2
    if [ "$ALL_FLAG" = "all" ]; then
       FIND_FLAG=
        ...
        FIND_FLAG="-maxdepth 1"
        FILTER=$1
    fi
    DEFCONFIG_ARRAY=($(pushd $BUILDROOT_DIR/configs 2>&1 >> /dev/null; find $FIND_FLAG -name '*_release_defconfig' | sed 's@./@@' | sed 's@_defconfig@@' | grep "$FILTER" | sort))
    DEFCONFIG_ARRAY_LEN=${#DEFCONFIG_ARRAY[@]}
...
#		fi
		...
		if [ -n "`echo $ANSWER | sed -n '/^[0-9][0-9]*$/p'`" ]; then
			if [ $ANSWER -le $DEFCONFIG_ARRAY_LEN ] && [ $ANSWER -ge 0 ]; then
				index=$((${ANSWER}))
                SELECTED_CONFIG=${DEFCONFIG_ARRAY[$index]}
                echo "select config: =$SELECTED_CONFIG="
                if [[ "$SELECTED_CONFIG" =~ "/" ]]; then
                  cp -a $BUILDROOT_DIR/configs/${SELECTED_CONFIG}_defconfig $BUILDROOT_DIR/configs/
                  SELECTED_CONFIG=${SELECTED_CONFIG##*/}
                fi
                echo "select config: =$SELECTED_CONFIG="
				TARGET_BUILD_CONFIG=`get_build_config ${SELECTED_CONFIG}`
                TARGET_BUILD_CONFIG=${TARGET_BUILD_CONFIG##*/}
				TARGET_DIR_NAME="${SELECTED_CONFIG}"
				TARGET_BUILD_TYPE=`get_target_build_type ${SELECTED_CONFIG}`
				TARGET_BOARD_TYPE=`get_target_board_type ${SELECTED_CONFIG}`
			else
				echo
...
function lunch()
{
	...
	if [ -n "$TARGET_BUILD_CONFIG" ]; then
		...
		if [ -d "$BUILDROOT_EXTERNAL_DIR" ]; then
			make O="$TARGET_OUTPUT_DIR" BR2_EXTERNAL="$BUILDROOT_EXTERNAL_DIR" "$TARGET_BUILD_CONFIG"_defconfig
		else
...
function apply_default_patches()
{
    if [ -d $LOCAL_DIR/aml-patches ] && [ -n "$(find $LOCAL_DIR/aml-patches -name '*.patch')" ]; then
        echo "Appling Patches in $LOCAL_DIR/aml-patches"
        $LOCAL_DIR/buildroot/build/aml-br-patch-workspace.sh -w $LOCAL_DIR/ -p $LOCAL_DIR/aml-patches || return 0
    else
	    return 0
    fi
}

function check_hifi_repo()
{
	if grep -q '^BR2_PACKAGE_AML_HIFI_RTOS_SDK=y$' $TARGET_OUTPUT_DIR/.config; then
		echo "Find HiFi config in .config file"
	else
		# HiFi is not enabled, just return here
		return
	fi
	if [ ! -d $LOCAL_DIR/vendor/amlogic/rtos/HiFiDSP_rtos_sdk  ]; then
		COLOR_RED="\e[31m"
		COLOR_NONE="\e[0m"
		echo -e "$COLOR_RED"
		echo  "=========================================================================================================================="
		echo "Can not find vendor/amlogic/rtos/HiFiDSP_rtos_sdk directory, so disable HiFi config to ensure the compilation OK."
		echo "If you need HiFi DSP function, you should apply the pull permission from Amlogic IT."
		echo "=========================================================================================================================="
		echo -e "$COLOR_RED""$COLOR_NONE"
		sed -i 's/^\(BR2_PACKAGE_AML_HIFI_RTOS_SDK=\)y/\1n/' $TARGET_OUTPUT_DIR/.config
	fi
}
